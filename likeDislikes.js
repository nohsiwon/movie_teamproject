...
function updateLikeDislikeUIOnLoad() {
  ...
  for (let i = 0; i < posts.length; i++) {
    const post = posts[i];
    // 각 댓글의 초기 좋아요와 싫어요 카운트를 화면에 업데이트
    updateLikeDislikeUI(post.id, post.likes || 0, post.dislikes || 0);
  }
}
...
function toggleLikeDislike(commentId, action) {
  let posts = JSON.parse(localStorage.getItem('posts')) || [];
  const comment = posts.find((post) => post.id === commentId);

  if (!comment) return; // 댓글이 없으면 종료
